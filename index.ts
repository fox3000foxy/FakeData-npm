...
export function buildCredibleEmailAddress(
    ...
    const domaines: any = (datasets as any).mailboxes;
...
// usernames handling
const usernamesTemplate: string[] = (datasets as any).usernames;
...
export function getContinent(countryCode: string): string {
    const continentsCountries: any = (datasets as any).continentsCountries;
...
const preferencesPublicitaires: any = (datasets as any).adChoices;
...
export function generateFakeProfile(params: {
    ...
    const person: { name: string; surname: string } = {
        ...
        surname: randomItem<string>(
            (datasets as any)[country.abbreviation].last as string[]
        ),
    };
    ...
    const password =
        randomItem<string>((datasets as any).common.passwords as string[]) +
        randomItem<string>((datasets as any).common.passwords as string[]) +
        range(100, 999);
    const salt = range(2, 8);
    const street = randomItem((datasets as any)[country.abbreviation].street);
    const city = randomItem((datasets as any)[country.abbreviation].cities);
    const state = randomItem((datasets as any)[country.abbreviation].states);
